export default function ProductCard(props) {
  ...
  const img =
    product.images[0] || product?.image || "https://via.placeholder.com/600x600?text=Product";
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            {labeled ? (
              <div className="text-xs text-slate-500 line-through">{labeled}</div>
            ) : null}
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <button className="rounded-xl bg-slate-900 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800 active:scale-[0.99]">
            View
          </button>
        </div>
      </div>
